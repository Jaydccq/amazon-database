...
TRUNCATE TABLE reviewvotes CASCADE; -- Added
...
-- Corrected: Include upvotes and downvotes columns
\copy reviews_feedbacks(review_id, user_id, comment, review_date, product_id, seller_id, rating, updated_at, upvotes, downvotes) FROM 'app/db/data/generated/Reviews_Feedbacks.csv' WITH (FORMAT CSV, NULL '', DELIMITER ',', HEADER true); -- Use NULL '' for potentially empty product/seller IDs

-- Added: Copy command for ReviewVotes table
\copy reviewvotes(user_id, review_id, vote_type, voted_at) FROM 'app/db/data/generated/ReviewVotes.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER true);
...
-- Corrected: Include status column
\copy cart_products(cart_id, product_id, seller_id, quantity, price_at_addition, added_at, status) FROM 'app/db/data/generated/Cart_Products.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER true);
...
\copy orders_products(order_id, product_id, quantity, price, seller_id, status, fulfillment_date) FROM 'app/db/data/generated/Orders_Products.csv' WITH (FORMAT CSV, NULL '', DELIMITER ',', HEADER true); -- Use NULL '' for potentially empty fulfillment_date
...
SELECT pg_catalog.setval('reviewvotes_vote_id_seq', COALESCE((SELECT MAX(vote_id) FROM reviewvotes), 1), true);
...
SELECT pg_catalog.setval('orders_products_order_item_id_seq', COALESCE((SELECT MAX(order_item_id) FROM orders_products), 1), true);
...
SELECT COUNT(*) AS review_vote_count FROM reviewvotes; -- Added count
...
\echo 'Data loading and sequence reset complete.'